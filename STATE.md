...
| L13 | 2026-06-13 | verify | ksg_te + ksg_robustness now DISPATCH to the GPU (gpu/ksg_gpu.py via ce_ksg_gpu_pairs in _ksg_core.R) when a CUDA device is present, governed-CPU fallback otherwise (default AUTO; CE_GPU=0 forces CPU). Bit-exactness gate gpu/equiv_gate.R: observed TE == engine .te() to <=7e-16 (6-dp emit identical) across lag1/lag2, k4/k5 — and observed TE is the ONLY thing the evals gate (ksg_te band; ksg_robustness rankings/Spearman), so GPU-default never moves a published number. KEY non-obvious bug found by the gate: the engine's .count_1d (1-D Z count, lag=1 only) tests membership via findInterval(v-eps) not &#124;dp-v&#124;<eps, so it INCLUDES a boundary point the abs-form excludes when &#124;dp-v&#124; rounds to exactly eps; replicated host-side (the 2-D .count_kd path already matches the GPU abs-form). Surrogate IAAFT runs on the host (the method is UNSEEDED, so p-values are non-reproducible in both paths); batched-FFT + a single batched (S x m) kernel launch per pair amortise launch/transfer overhead. MEASURED (12 pairs x B=99, this box): GPU 82s (~1 core) vs governed CPU 289s (~7 cores) = 3.5x faster AND frees the CPU — full-g20 ksg_te ~35 min GPU vs ~122 min CPU. Lesson: when porting a published estimator to a faster device, gate on the estimator's OWN output incl. its FP boundary quirks, not the textbook form. Distillation PENDING (PI gate). | pending |
