...
| **Claude on Vertex** (Model Garden, `multimodel`) | **API ENABLED, MODEL-GARDEN ENABLEMENT PENDING** | `aiplatform` enabled; Anthropic publisher models require a one-time Model Garden enable per project+region | second-opinion reviewer for `/api/research`; code-complete + flag-OFF; marked hole below |
...
- **Claude-on-Vertex (`multimodel`) — ENABLED 2026-06-16, but online-prediction QUOTA is the gate:** the PI enabled Anthropic Claude (Opus 4.8) in Model Garden (Marketplace purchase = Anthropic terms accepted), so access is granted. But a live `rawPredict` smoke test returned **HTTP 429 `RESOURCE_EXHAUSTED`** — the `global_online_prediction_requests_per_base_model` quota for `anthropic-claude-opus` is **0 / unprovisioned**, uniform across the `global` endpoint AND every regional endpoint tested (us-east5/us-east1/europe-west1/asia-southeast1 → 429; us-central1 → 400). **Access ≠ quota:** the remaining gate is a **quota-increase request** to Google (console Quotas page, or the Cloud Quotas API). The global-host fix is verified — the 429 proves the corrected unprefixed `aiplatform.googleapis.com` + `locations/global` host reaches the model. Default model `claude-opus-4-8` (PI-enabled); env-overridable (`CE_MULTIMODEL_MODEL`, `CE_MULTIMODEL_LOCATION`). Code-complete and governed (flag `CE_CAP_MULTIMODEL` default OFF, `CE_MULTIMODEL_PER_DAY` ceiling, governance eval 10/10); NOT-LAUNCHABLE until quota is granted + a live smoke returns `code:OK`. Flipping the flag ON and deploying is PI-gated (Invariants 11/15/16).
